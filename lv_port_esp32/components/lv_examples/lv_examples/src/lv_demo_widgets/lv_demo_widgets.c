...
static uint32_t bank = 1;
static uint32_t mode = 1;
static uint32_t track = 1;
...
static void mode_updater(lv_task_t * t)
  ...
  static char buf[64];
  if (mode == 1){
    lv_snprintf(buf, sizeof(buf), "B", mode);
  }
  if (mode == 2){
    lv_snprintf(buf, sizeof(buf), "K", mode);
  }
  if (mode == 3){
    lv_snprintf(buf, sizeof(buf), "S", mode);
  }
  if (mode == 4){
    lv_snprintf(buf, sizeof(buf), "N", mode);
  }
  if (mode == 5){
    lv_snprintf(buf, sizeof(buf), "L", mode);
  }
...
static void arc_anim(lv_task_t * t)
    {
      /*
      for(int i = 0; i < 8; ++i){
        if (arcValues[i] == arcChecks[i]){

        }
        else{
        lv_arc_set_value(arc1, arcValues[i]);

        arcChecks[i] = arcValues[i];
        }
      }
      */
